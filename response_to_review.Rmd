...
- References
    ...
    - buja et al (1986) this is a conference proceedings series; capitalize similar to a journal - I think this needs to be fixed via bibstyle
    - carr add page numbers (or is this the previous ref) - I think this is for the previous reference, again it is there in the bib file, to show it the bibstyle should be formatted accordingly for proceeding entries
    - chambers et al list the editors - I don't think there was any 'scientific' editor on this book, didn't find any relevant person to list here ...
    - coleman add when accessed - this was taken from animation package so I got the date when the rda file was added to its github repo and added it here...
    ...
    - horst capitalize both "A" as these seem to be geographic names; just "R package" once - fixed capitalization, but repetition needs to be fixed in bibstyle
    - hotelling should be "417-441" - fixed (plus full journal name instead of abbreviation)
    - huber is there a URL where this can be obtained? - could not find it via google at least...
    ...
    - swayne DOI - I have no idea why it is not printing the DOI for the XGobi reference...
    - swayne et al DOI; XGobi (as a software name); issue ? -fixed
    - tukey editors? - added something based on web search, but not sure I got the right names..
    - tukey pages, city, publisher ?; city? - as above ..
    - van der maaten they list this as 9(86) on their web page - fixed
    - wegman list editors only once; page numbers; is there a volume ? - these are Interface (conference proceedings) - problem with showing these information for proceedings entry, again bibstyle issue?
