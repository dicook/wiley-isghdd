...
The classic tour displays work with projections, and interactive systems like GGobi also provide the option of adding sectioning information via linked brushing. Sectioning means we select points that fall in a section of the full parameter space, for example for the purpose of highlighting them in a tour display. This often reveals complementary information, and combining sections and projections can for example provide insights into the dimensionality of a data structure [@Furnas1994-dz].
...
The slice tour can reveal concave or non-linear structures obscured in projections, as well as small structures hidden near the center of a distribution. An example showing snapshots of the slice tour of points distributed on the surface of geometric shapes is shown in Fig. \ref{fig:tour-slice}. As with projections, an index function may be used to select target planes showing interesting slices of the data, and we can define a section pursuit guided tour [@Laa2020-ni].
...
\caption{When a user brushes data on the left panel, points that fall inside the region are highlighted in orange. The view on the right responds by higlighting the corresponding points.}
\label{fig:brushing}
\end{figure*}
