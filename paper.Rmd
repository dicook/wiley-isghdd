...
\begin{figure*}
   \centering
         \includegraphics[width=0.5\textwidth]{figures/liminal-stop-pause-refresh.png}
\caption{The clustering example in figure \ref{fig:tsne-tour} using the liminal interface. Here a grand tour is displayed on the right hand side, with buttons allowing users to play, pause and refresh the tour animation.}
...
\caption{Selected frames from using a grand tour of mouse retina single cell RNA-seq data with the radial transformation described in \cite{Laa2020-uv}. Here the sage display was used to identify and verify cluster sepearation between the three highlighted clusters estimated from a clustering algorithm.}
