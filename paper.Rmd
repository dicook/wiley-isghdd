...
The classic tour displays work with projections, and interactive systems like GGobi also provide the option of adding sectioning information via linked brushing. Sectioning can be considered as providing complementary information, and in combination with projections we can for example gain insights into the dimensionality of a data structure [@Furnas1994-dz]. Sectioning a high-dimensional space in systematic manner (without interactive selection of data sections) is challenging, since there is a lot of freedom in choosing a section. A new approach is implemented in the slice tour display [@Laa2020-js], which is using the distance of each from the current projection plane (typically placed such that it passes through the data mean) to highlight points that are nearby and fade out points further from the plane.
...
\begin{figure*}[!ht]
   \centering
         \includegraphics[width=0.45\textwidth]{figures/ggobi_book/brush_query.pdf}
         \includegraphics[width=0.45\textwidth]{figures/ggobi_book/brush_response.pdf}
