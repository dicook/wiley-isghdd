...
Various displays can be used to show projections in 1D, 2D and higher dimensions. A 1D projection displays the data in a histogram, while 2D projections are displayed as scatterplots,  analogous to a shadow puppet being projected onto a wall. Cook implemented 3D tours in VRGobi [@vrgobi;@Nelson1999XGobiVT].  Higher dimensional projections can be shown with multivariate displays like SPLOMs or parallel coordinates. Figure \ref{fig:display} shows a variety of displays for projections and these displays are useful to show the non-normal distributions in the projected data, clustering structure, and multivariate relationship between variables. It is possible to define tours for more complex data also. For example, @Martinez2013 proposed the image grand tour for multivariate spatial raster data, which displays a linear combination of variables at each spatial pixel. 
