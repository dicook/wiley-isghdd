...
Instead of randomly generating targets, guided tour [@Cook1995-ae] finds its targets with a technique called projection pursuit [@Friedman1974-ck].  In projection pursuit, an index function is a surjective mapping from the projection $Y$ in $\mathbb{R}^{n \times d}$ to a scalar in $\mathbb{R}$. Guided tour iteratively optimises the index function in the projection pursuit and produces the most interesting projection according to the index measure. The right panel of Figure \ref{fig:notation-target-gen} shows an example of a guided tour path. 
...
\caption{Still views of the slice tour display showing geometric shapes: Roman surface (left), 4D torus (middle) and a 6d cube (right). Adapted from Laa, Cook and Valencia (2020), Fig. 4.}
