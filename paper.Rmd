...
A tour path, that is a sequence of projections, can be generated by geodesic interpolation between a set of target planes (Figure \ref{fig:tour-path}). Different methods for choosing target planes provide different tour paths. The grand tour [@Asimov1985-xr] is generated using randomly selected target. A guided tour [@Cook1995-ae] generates targets as a sequence of interesting projections $A_i$ by applying projection pursuit optimization on a predefined criteria $c(\cdot)$ of ``interestingness'' of a projection, $argmax_A c(XA)$. A local tour (available originally in [@Buja1986-la] rocks back and forth from a particular plane $A$ to randomly chosen targets $A_i$ in a small neighborhood $\{A_j |\; d(A_j, A) < \epsilon \}$. A little tour (available originally in [@Buja1986-la] navigates through all of the coordinate planes, for example, for 2D projections $\{A_{ik} = 1, A_{lj} = 1, \forall i, j, k, l = 1, ..., p; i\neq j\}$.
